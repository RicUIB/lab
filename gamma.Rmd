...
-\int_0^{\infty} z\cdot x^{z-1} \cdot \left(-e^{-x}\right) dx=\\
&&
\lim_{x\to\infty}\left(-x^z\cdot e^{-x}\right)
+
z\cdot \int_0^{\infty} x^{z-1} \cdot e^{-x}.
\end{eqnarray}
$$
...
La definición de factorial de un un número natural $n\in\mathbb{N}$,  e ¡¡obviamente!!! recursiva
